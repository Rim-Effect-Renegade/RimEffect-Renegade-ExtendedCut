...
namespace RimEffectExtendedCut
{
    public class PatchOperation_SettingActive : PatchOperation
    {
        ...
        public override bool ApplyWorker(XmlDocument xml)
        {
            ...
            ModContentPack modContentPack = RimEffectExtendedCutMod.mod.Content;
            for (int i = 0; i < settings.Count(); i++)
            {
                if (!RimEffectExtendedCutMod.settings.IsValidSetting(settings[i]))
                {
                ...
                if (RimEffectExtendedCutMod.settings.GetEnabledSettings.Contains(settings[i]))
                {
                    flag = true;
                    break;
                }
