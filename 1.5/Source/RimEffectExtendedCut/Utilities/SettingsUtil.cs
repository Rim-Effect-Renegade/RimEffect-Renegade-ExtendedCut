...
namespace RimEffectExtendedCut
{
    public static class SettingsUtil
    {
        ...
        public static void SettingsDropdown(this Listing_Standard listing, string name, string explanation, ref RimEffectExtendedCutSettingsPage value, float width)
        {
            float curHeight = listing.CurHeight;
            Rect rect = listing.GetRect(Text.LineHeight + listing.verticalSpacing);
            Text.Font = GameFont.Small;
            ...
            if (Widgets.ButtonText(rect2, value.ToString().Replace("_", " "), true, true, true))
            {
                ...
                List<RimEffectExtendedCutSettingsPage> enumValues = Enum.GetValues(typeof(RimEffectExtendedCutSettingsPage)).Cast<RimEffectExtendedCutSettingsPage>().ToList();
                foreach (RimEffectExtendedCutSettingsPage enumValue in enumValues)
                {
                    list.Add(new FloatMenuOption(enumValue.ToString().Replace("_", " "), delegate ()
                    {
                        RimEffectExtendedCutMod.mod.currentPage = enumValue;
                    }));
                }
